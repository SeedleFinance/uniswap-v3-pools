...
const Modal = ({ title, children, className, onClose }: Props) => {
  ...
  return ReactDOM.createPortal(
    <div
      ...
      onKeyUp={handleKeyup}
    >
      <div className="fixed h-screen w-full bg-gray-300 bg-opacity-75"></div>

      <div
        ref={containerEl}
        tabIndex={0}
        className={`w-1/2 max-w-screen-sm bg-white shadow-md p-4 z-10 rounded-lg focus:outline-none ${className}`}
      >
